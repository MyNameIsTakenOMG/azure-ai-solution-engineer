...
## Assessments
 ...
 - 3rd test
   ...
   - The Azure AI Custom Vision service only exports compact domains, and the models generated by compact domains are optimized for the constraints of real-time classification on mobile devices.
   - Substitution errors are due to the model needing more training on custom product names and people names.
   - deletion errors: Overlapping speakers define
   - insertion errors: People talking in the background are detected
   - If the language of the content in the source document is known, it is recommended to specify the source language in the request to get a better translation.
   - between 40 and 60 indicates a high-quality translation for bilingual evaluation understudy(BLEU) for azure ai translator custom model
   - for training conversational language understanding(CLU), there are standard and advanced methods
   - The precision metric reveals how many of the predicted classes are correctly labeled.
   - Recall measures the model's ability to predict actual positive classes.
   - F1 score is a function of precision and recall.
   - BLEU is from the Azure AI Translator service.
   - with CLU, there is no need to use multiple projects for a model. For example, you can train a model in English and query it in German.
   - Active learning suggestions are not in real time. There is an approximate delay of 30 minutes before suggestions show on this pane.
